...
- **Génération en masse** : `python3 engine/autoblog.py --count N --days M` génère N articles (rotation des 5 catégories) avec dates échelonnées sur M jours. Génération longue (~1 min/article) — lancer avec `nohup ... > /tmp/gen.log 2>&1 &` et surveiller `ls site/src/content/blog/*.md | wc -l`. Vérifier après coup : pas de hors-sujet ("chat"=discussion), variété des catégories.
